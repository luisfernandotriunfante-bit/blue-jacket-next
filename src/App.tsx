...
export function App() {
  ...
  async function processHistory() {
    const salesFiles = [...(historySlots.sales ?? []), ...(historySlots.summary ?? [])].map(f => rawFiles[f.id]).filter((f): f is File => Boolean(f)); if (!salesFiles.length) return
  ...
  async function processAll() {
    ...
    if ([...(historySlots.sales ?? []), ...(historySlots.summary ?? [])].some(f => rawFiles[f.id])) toRun.push(processHistory())
