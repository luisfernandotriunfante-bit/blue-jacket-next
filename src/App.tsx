...
const movementSources = [
  { id: 'sales', label: 'Vendas atuais' }, { id: 'cuts', label: 'Cortes por cliente' }, { id: 'entries', label: 'Entradas por nota' },
] as const
...
export function App() {
  ...
  const audit = useMemo(() => [...buildAudit(files).filter(item => (item.area !== 'clientes' || clientBase.length === 0) && (item.area !== 'produtos' || productBase.length === 0) && (item.area !== 'historico' || historyBase.length === 0) && (item.area !== 'movimentacoes' || movementBase.length === 0)), ...clientAudit, ...productAudit, ...historyAudit, ...movementAudit], [files, clientAudit, clientBase.length, productAudit, productBase.length, historyAudit, historyBase.length, movementAudit, movementBase.length])
