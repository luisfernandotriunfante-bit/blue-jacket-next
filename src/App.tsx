...
export function App() {
  ...
  function downloadProductExcel() {
    const records = productBase.map(product => ({ Agrupamento: product.groupName ?? '', Família: product.groupFamily ?? '', Código: product.internalCode ?? '', 'Código fabricante': product.manufacturerCode ?? '', EAN: product.ean ?? '', Descrição: product.description ?? '', Embalagem: product.package ?? '', Marca: product.brand ?? '', Categoria: product.category ?? '', Subcategoria: product.subcategory ?? '', 'Unidades por caixa': product.unitsPerBox ?? '', Disponível: product.availableStock ?? '', Estoque: product.totalStock ?? '', Reservado: product.reservedStock ?? '', Bloqueado: product.blockedStock ?? '', Avariado: product.damagedStock ?? '', 'Preço de venda': product.sellerPrice ?? '', 'Quantidade em carteira': product.inTransitQuantity ?? '', 'Valor em carteira': product.inTransitValue ?? '', Status: product.status, Departamento: product.department ?? '', Linha: product.productLine ?? '', Comprador: product.buyer ?? '', Fornecedor: product.supplierName ?? '', 'Custo real': product.realCost ?? '', 'Custo financeiro': product.financialCost ?? '', 'Venda mês': product.monthlySales ?? '', 'Giro diário': product.dailyTurnover ?? '', 'Preço base indústria': product.industryBasePrice ?? '', 'Peso líquido unitário': product.netWeightUnit ?? '' }))
    const essential = new Set(['Agrupamento', 'Família', 'Código', 'Descrição', 'Disponível', 'Estoque', 'Preço de venda', 'Quantidade em carteira', 'Valor em carteira', 'Status'])
    const columns = Object.keys(records[0] ?? {}).filter(column => essential.has(column) || records.some(record => record[column as keyof typeof record] !== ''))
