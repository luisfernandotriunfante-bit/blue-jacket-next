...
export async function processProductMotor(files: File[]): Promise<ProductMotorResult> {
  ...
  const sortimentSubBrands = new Map<string, string>()
  ...
  for (const file of files) for (const { rows } of await rowsFrom(file)) {
    ...
    if (hPricesheet >= 0) { let count = 0; for (const row of rows.slice(hPricesheet + 1)) { const key = code(row[0]); if (!key || prices.has(key)) continue; const sem = number(row[4]); const com = number(row[5]); if (sem === undefined && com === undefined) continue; const upb = number(row[6]); prices.set(key, { internalCode: key, ean: code(row[1]), description: text(row[2]), sellerPriceWithoutTax: sem, sellerPrice: com, ...(upb !== undefined && { unitsPerBox: upb }), sources: ['Planilha de preços'] }); count++ } results.push(audit('prod-pricesheet', 'Planilha de preços reconhecida', `${count.toLocaleString('pt-BR')} preços prontos para juntar.`, 'Preço sem ST (PREÇO UND), com ST (PREÇO UND ST) e unidades por caixa foram lidos.')); continue }
    ...
    if (hSortiment >= 0) { let count = 0; for (const row of rows.slice(hSortiment + 1)) { const key = code(row[0]); if (!key || sortiment.has(key)) continue; sortiment.set(key, { manufacturerCode: key, sortimentStatus: text(row[16]), lifestageStatus: text(row[18]), sources: ['Sortimento'] }); count++ } results.push(audit('prod-sortiment', 'Sortimento reconhecido', `${count.toLocaleString('pt-BR')} itens no sortimento.`, 'O status e ciclo de vida de cada produto foram lidos.')); continue }
    if (hSortBase) { const { h, codCol, chanStart } = hSortBase; const hRow = rows[h]; const channels: [number, string][] = []; for (let i = chanStart; i < hRow.length; i++) { const ch = String(hRow[i] ?? '').replace(/\r?\n/g, ' ').trim(); if (ch) channels.push([i, ch]) } let subCol = -1; for (let i = 0; i < (hRow as unknown[]).length; i++) { const n = norm(hRow[i]); if (n === 'subbrand' || n === 'submarca') { subCol = i; break } } let count = 0; for (const row of rows.slice(h + 1)) { const key = code(row[codCol]); if (!key) continue; if (subCol >= 0 && !sortimentSubBrands.has(key)) { const sub = text(row[subCol]); if (sub) sortimentSubBrands.set(key, sub) } if (sortimentChannels.has(key)) continue; const vals: Record<string, number> = {}; for (const [ci, ch] of channels) { const n = Number(String(row[ci] ?? '').trim()); if (Number.isFinite(n) && n > 0) vals[ch] = n } if (Object.keys(vals).length > 0) { sortimentChannels.set(key, vals); count++ } } if (count > 0) results.push(audit('prod-sortiment-channels', 'Canais de sortimento reconhecidos', `${count.toLocaleString('pt-BR')} itens com canais de recomendação.`, 'Os formatos de loja recomendados foram lidos por produto.')); continue }
  }
